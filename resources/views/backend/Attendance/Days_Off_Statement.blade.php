            ...
            <form class="form-horizontal">
                <div class="card-body">
                    ...
                    <div class="card-footer text-center">
                        <button type="submit" class="btn btn-info search_in_Days_Off_Statement">بحث</button>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        <tbody>
                            @for ($dp = 0; $dp < count($data); $dp++)
                                <tr class="{{ $data[$dp]->id }}tr">
                                    <td>{{ $data[$dp]->name }}</td>
                                    <td>{{ $data[$dp]->date }}</td>
                                    <td>{{ $data[$dp]->is_annual_holiday }}</td>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Get Data
            function GetRows() {
                axios.get("{{ route('list_of_Days_Off_Statement') }}")
                    .then(function(response) {
                        ...
                        response.data.forEach(element => {
                            ...
                            data += "<td>" + element.is_annual_holiday + "</td>";
                            data += "<td>";
            ...
            //search
            $('.search_in_Days_Off_Statement').on('click', function(e) {
                e.preventDefault();
                ...
                var URL = "{{ route('search_in_Days_Off_Statement') }}";
                axios.post(URL, {
            ...
            $('table').on('click', '.delete', function(e) {
                e.preventDefault();
                var id = $(this).attr('id');
                var URL = "{{ route('delete_Days_Off_Statement') }}";
                axios.post(URL, {
                        id: id
                    })
                    .then(function(response) {
