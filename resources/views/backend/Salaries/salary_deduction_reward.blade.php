...
@section('title', ' الخصوم و المكافأت ')
...
<div class="callout callout-success">
    ...
    <div>
        <select name="" id="" class="form-control text-center List_Search">
            ...
            <option value="date_name"> التاريخ و الاسم </option>
        </select>
    </div>
    <form method="post">
                        ...
                        <tbody class="text-center">
                            @for ($dp = 0; $dp < count($data); $dp++)
                                <tr class="{{ $data[$dp]->id }}tr">
                                    <td>{{ $data[$dp]->date }}</td>
                                    <td>{{ $data[$dp]->employee }}</td>
                                    <td>{{ $data[$dp]->count_days }}</td>
                                    <td>
                                        @if ($data[$dp]->Salary_Deduction == 1)
                                            <i class="fa fa-check" aria-hidden="true"></i>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($data[$dp]->Salary_reward == 1)
                                            <i class="fa fa-check" aria-hidden="true"></i>
                                        @endif
                                    </td>
                                    <td><button class="btn btn-danger btn-sm delete" id="{{ $data[$dp]->id }}"><i
                                                class="fas fa-trash p-1"></i>Delete</button></td>
                                </tr>
                            @endfor
                        </tbody>
                    </table>
    ...
    <script>
        function GetRows(empData) {
            ...
            for (var emp = 0; emp < empData.length; emp++) {
                ...
                data += "<td>" + empData[emp].employee + "</td>";
                data += "<td>" + empData[emp].count_days + "</td>";
                data += "<td>";
                if (empData[emp].Salary_Deduction == 1) {
                    data += "<i class='fa fa-check' aria-hidden='true'></i>"
                }
                data += "</td>";
                data += "<td>";
                if (empData[emp].Salary_reward == 1) {
                    data += "<i class='fa fa-check' aria-hidden='true'></i>"
                }
                data += "</td>";
                data += "<td><button class='btn btn-danger btn-sm delete' id='" + empData[emp].id +
        ...
        $('tbody').on('click', '.delete', function(e) {
            ...
            var URL = "{{ route('Salary_Deduction_Reward_Delete', ':id') }}".replace(':id', id);
            axios.get(URL, {
                    id: id
                })
        ...
        $('form').on('click', '.Search_Btn', function(e) {
            ...
            if ($('.List_Search').val() == 'date') {
                Query = `  salary_deduction_reward.date BETWEEN '${from_date}' AND '${to_date}' `;
                Params = {
                ...
                Query = `  salary_deduction_reward.date BETWEEN '${from_date}' AND '${to_date}' AND salary_deduction_reward.emp_id =  '${emp_name}' `;
                Params = {
                    'status': $('.List_Search').val(),
                    'Query': Query,
                };
            ...
            var URL = "{{ route('Salary_Deduction_Reward_Search') }}";
